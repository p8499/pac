 ...
 */
public class UninstallPostgresqlGenerator extends Generator0 {
    public UninstallPostgresqlGenerator(Map project) {
        super(project);
    ...
    public File getPath(File folder) {
        return new File(folder, "uninstall_postgresql.sql");
    }

    @Override
    public String getContent() throws IOException {
        Template template = getVelocityEngine().getTemplate("com/p8499/paca/templates/database/uninstall_postgresql.vm");
        Writer bufferWriter = new StringWriter();
        template.merge(getContext(), bufferWriter);
        bufferWriter.flush();
        bufferWriter.close();
        TGSqlParser tgSqlParser = new TGSqlParser(EDbVendor.dbvpostgresql);
