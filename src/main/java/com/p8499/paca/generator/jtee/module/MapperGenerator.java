 ...
 */
public class MapperGenerator extends Generator1 {
    public MapperGenerator(Map project) {
        super(project);
    ...
    public File getPath(File folder, int index) {
        ...
        String packageName = (String) ((Map) project.get("envJtee")).get("packageMapper");
        Map module = (Map) ((List) project.get("modules")).get(index);
        String datasource = (String) module.get("datasource");
        String className = (String) module.get("jteeMapperAlias");
        return new File(folder, "src/main/java/" + packageName.replace(".", "/") + "/" + datasource + "/" + className + ".java");
    }

    @Override
    public String getContent(int index) throws IOException, FormatterException {
        getContext().put("index", index);
        Template template = getVelocityEngine().getTemplate("com/p8499/paca/templates/jtee/module/mapper.vm");
